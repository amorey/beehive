...
| event watch | one object's event log above a cursor, for each live `EventsWatch` | what the log has grown by, and a commit wakes it before the tick | `WithWatchFloorInterval`, default 30s |
