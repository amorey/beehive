...
// ErrInvalidName is returned by the name-keyed calls when the name is the empty
// string. The client rejects it before any store work; the store rejects it too,
// since Store is a public extension point and a "" row is one no name-keyed call
// could address.
var ErrInvalidName = storeapi.ErrInvalidName

// ErrNameTaken is returned by Create when the name is already held, by a live row
// or by a deletion-pending one. GetOrCreate returns the existing row instead; this
// is Create's answer, and what a caller generating names retries on.
