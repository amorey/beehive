...
- **Dependency-wake losses are repaired by replaying a `resource_version` watermark**, not by escalating a periodic pass. The waker holds the highest version it has processed and, on a failed lookup or a re-established subscription, replays everything above it (`ObjectWritesListSince`, paged) through the same wake path — O(changes missed). It owns its own resubscribe loop with a backoff seam, since a cursor is not a trigger. The old tick escalation is deleted: its flags were read only inside the catchup tick, which the default config may not have. → [ADR](docs/adr/2026-07-27-waker-watermark-replay.md)
