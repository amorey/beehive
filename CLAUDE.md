...
- **One process, one `Beehive`, and it is the store's only writer.** Two
  ...
  second `Beehive` over a running database is `ErrStoreInUse`, keyed on
  `Store.Identity`, so two stores over one file collide too — and the claim is
  held until the loops it cancelled have stopped, so a blown stop deadline
  narrows the window rather than opening one. Documented only across
...
- **Reconcile is not transactional.** Each `ControllerClient` write commits on
  ...
  **A converged spec write still takes its transaction, and still reads before it
  writes**: the lock-free probe in front and the read folded into the `UPDATE`
  were both measured and declined, the second because a statement matching no row
  costs more than the read it replaces.
  → [ADR](docs/adr/2026-08-19-a-spec-write-takes-its-transaction-unconditionally.md),
  [ADR](docs/adr/2026-08-20-a-spec-write-reads-before-it-writes.md)
