...
const (
	...
	// defaultUnsettledRequeue is when a bare Unsettled() comes back. A bare
	// result must schedule something: the unsettled listing gates on the
	// generation, so an object that declines to settle without having moved its
	// generation is in no listing. Matches the owed pass, which is the rate the
	// listed half of that population already converges at.
