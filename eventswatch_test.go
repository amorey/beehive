...
func TestWatchEventsSnapshotCarriesItsPosition(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Probing"}))
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Connected"}))
...
func TestWatchEventsRedeliversAnExtendedRun(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, probe))
...
func TestWatchEventsDeliversAscending(t *testing.T) {
	...
	for _, reason := range []string{"R1", "R2", "R3"} {
		require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: reason}))
	}
	var last int64
...
func TestWatchEventsFiltersTheTail(t *testing.T) {
	...
	for range 3 {
		require.NoError(t, cc.at(obj.ID).AddEvent(ctx,
...
func TestWatchEventsLimitBoundsTheSnapshotOnly(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "R4"}))
...
func TestWatchEventsResumesFromAPosition(t *testing.T) {
	...
	_, _, client, cc := watchFixture(t)
	obj := mustCreate(t, ctx, client, uniqueName(), cSpec{Val: "a"})
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Seen"}))

	first, err := client.WatchEvents(ctx, obj.ID)
	require.NoError(t, err)
	checkpoint := first.ResourceVersion

	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Missed"}))
...
func TestWatchEventsResumeBelowTheHorizonIsRefused(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Old"}))
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Newer"}))
...
func TestWatchEventsHorizonIsPerTimeline(t *testing.T) {
	...
	store, _, client, cc := watchFixture(t)
	obj := mustCreate(t, ctx, client, uniqueName(), cSpec{Val: "a"})
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Category: "quiet", Type: EventNormal, Reason: "Q1"}))
...
func TestWatchEventsWaitsForAnObjectThatDoesNotExistYet(t *testing.T) {
	...
	require.NoError(t, cc.at(later.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Started"}))
...
func TestWatchEventsIsNotKindScoped(t *testing.T) {
	...
	require.NoError(t, otherCC.at(foreign).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Started"}))
	...
	require.NoError(t, otherCC.at(foreign).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Probing"}))
...
func TestWatchEventsSurvivesReadFailures(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Recovered"}))
...
func TestWatchEventsKeepsAnUndeliveredRunOwed(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Undelivered"}))
	waitClosed(t, chanAfter(store.eventsListed, 1), "the log read that found the run")
	cancel()
...
func TestWatchEventsLeavesNothingBehind(t *testing.T) {
	...
	require.NoError(t, cc.at(obj.ID).AddEvent(ctx, EventSpec{Type: EventNormal, Reason: "Unheard"}))
