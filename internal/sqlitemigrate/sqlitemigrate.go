...
// PoolOptions configures OpenPool. MaxConns is required.
type PoolOptions struct {
	// MaxConns caps the pool: 1 for a writer pool, larger for a WAL reader pool.
	MaxConns int

	// Reader opens a WAL reader pool rather than the writer pool: query_only, no
	// _txlock, and connections that are never retired.
	Reader bool
}
...
func OpenPool(path string, opts PoolOptions) *sql.DB {
	...
	if opts.Reader {
		...
		// yet and after an unclean close. Without _txlock, BeginTx here is
		// DEFERRED rather than a write lock the connection cannot have.
	...
	if opts.Reader {
		// Never retired: attaching to a WAL database blocks while a writer holds a
		// transaction, so a re-attach costs the wait this pool exists to avoid.
