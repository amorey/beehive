...
| `ListPendingWakeIDs` (catchup tick) | only if the edge was declared against an already-moved target, a different case |
...
`ListUnsettledIDs` + `ListPendingWakeIDs` — structurally cannot see a settled
...
`Start` runs one waker over one subscription (`startDependencyWaker`,
`beehive.go:320`). Routing needed no change: it was never keyed on the
subscription — `wakeDependentsBatch` (`beehive.go:402`) enqueues each dependent
...
kinds currently referenced by an edge (changes on every `AddDependency`, so it is
...
for the whole table and `GetObject(ctx, id)` takes no `GroupKind`, so a global
...
The hub value is `pendingChange{typ, rv}` (`sqlite/watch.go:124`) and the stream
delivers `ObjectChange{ID, Type}`. This stream sees every write in the process, so
...
lagging consumer never saw intermediate states: `mergeChange` keeps one value per
...
architecture already requires. (The wire-facing analogue, `Client.Watch`'s
...
`wakeDependentsBatch` resolves the whole batch in one `GroupIncomingRefsByID`.
...
| option | why not |
|---|---|
| Subscribe per kind reachable through a `depends_on` edge | The set is dynamic: an edge declared at runtime introduces a kind nothing is watching, so it needs re-subscription on `AddDependency`. Correct only if that path is never missed. |
...
| Reject `AddDependency` against a client-only target | Converts a silent half-working case into a loud one, but makes a runtime error of something that mostly works, and the registered set is only known between `Register` and `Start`, so the check cannot sit at the natural place. Worth considering as a *warning*. |
...
| Buffered channel of single changes instead of receiver-side batching | De-conflates on exit from the receiver and (under the old `RawChange` value) pinned blobs. Pinned by `TestWatchObjectChangesCoalescesRepeatWrites`, which fails under that design. |
...
K independent wakers became one. A slow `GroupIncomingRefsByID` — itself queued
...
`hubs[gk]` (carrying `RawChange`) and `changeHub` (carrying `pendingChange`) are
...
`annihilatingMerge`'s `preserve` is now required rather than nilable. Deleting
...
red/green cycles: the global hub, `WatchObjectChanges`, the batch drain, the
conflation pins, stream shutdown, `wakeDependentsBatch`, then the single
...
`ChangeRef` → `ObjectChange` rename, and the removal of `annihilatingMerge`'s
dead branch.
