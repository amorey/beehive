...
`DeletionRequestsCreate`/`DeletionRequestsCreateByName` return
`DeletionRequestResult.Unblocked`; `DeletionRequestsCreateFromOwner` returns
...
this is that index's second consumer. Route 2's per-target `ObjectsGetMeta` was
...
- *The target is deletion-pending.* A live target was never blocked, and
  ...
  spin.
- *The relation is `depends_on`.* `EdgesHasIncoming`'s discount is specific to
...
check would cost an `EdgesHasIncoming` per target to save a dispatch the collect
already re-derives.
...
`DeletionRequestsCreate`, `DeletionRequestsCreateByName` and
`DeletionRequestsCreateFromOwner` change signature — a break in the exported
`Store`. Result structs rather than more return values, as the
[write-shapes ADR](2026-07-30-store-write-shapes.md) settled for
`EdgesAddResult` and `EdgesDeleteResult`; `DeletionRequestsCreateByName` would
...
- **`ObjectsGetMeta` per target, as route 2 does.** Rejected on cost: N+1
