# The name is the `Client` API's key; the id is the store's key
...
Every object could be named, but nothing required it. `Create` took the name through
`WithName`, an options bag, so "no name" was representable and the column was
...
hold an id to do ordinary work, and the name was reachable only through the two
`…ByName` methods that had been added where the ergonomics hurt most.

The README defended this with a real property: *"Everything after the create takes an
`ObjectID`, so a delete and recreate under the same name can't make you act on the
wrong row."* Names are reusable by design — a rename is delete+recreate — so a
name-keyed API can hand a caller a different incarnation than the one it read. That
is a genuine hazard, and any change here has to answer it rather than ignore it.

## Decision

Key the user-facing CRUD surface on the name, and make the name required.

- `Create` takes the name positionally; `WithName` is deleted. A required field
  passed through an options bag is the wrong shape — the bag can express an absence
  the argument cannot, which is exactly how a NULL name got written. Deleting it also
...
- `GetByName`/`DeleteByName` lose the suffix; the id-keyed forms they displace gain
  `ByID`. `Update` gains a name form and an `UpdateByID` sibling — it had no `ByName`
...
- `name TEXT NOT NULL`, and `Name` is a `string` on `Object`, `RawObject` and
  `ObjectsCreateInput`.
- `""` is rejected with `ErrInvalidName`, by the writes and the reads alike.
...
reconcile — which is what a name-keyed call does, and what made `GetOrCreate` and
the name-keyed delete worth adding in the first place. "Finish what I read a moment
ago" is a statement about an *incarnation*, and that is what an id is for.

Two things make the split safe:

1. **Every name-keyed write resolves and writes in one transaction.** Never two store
   ...
   narrow anyway: a tombstone holds the name's `UNIQUE` constraint until GC clears
   finalizers, so opening it takes a full collect *plus* a new create.
2. **The rename cannot fail silently.** `ObjectID` is an alias for `int64` and a name
...
- **ABA safety.** `objects.id` is `AUTOINCREMENT` precisely so ids are never reused,
  which is what makes stale edge targets impossible by construction. Name-keyed edges
...
- **Key width in the scan-hot tables.** An id is a varint, typically 1–3 bytes; a
  name key is `(group, kind, name)`, tens of bytes, on every row of `edges` (twice),
...
`storeapi.Store` also keeps `ObjectsGetByName` / `DeletionRequestsCreateByName` /
`ObjectsUpdateSpecByName` under those names. There the id really is the key and the
name methods really are the qualified variant; renaming would invert the convention
against the thing being named.

### Why `ObjectsUpdateSpecByName` reads before it writes

The obvious rule — "put the name in the `WHERE` clause, never resolve then write" —
generalises `DeletionRequestsCreateByName` one step too far. A spec write **must**
...
`UPDATE … WHERE name = ?` has nothing to compare against.
...
`""` is the one rule beehive enforces on an otherwise opaque key. Making the name
...
name at least addresses the row its author meant.

A new sentinel rather than `ErrInvalidOption`, whose godoc scopes it to option
values: the name is a positional argument on all four writes. The reads reject it
too, because `ErrNotFound` would send the caller hunting for a missing row when what
is missing is a config value, and the name-keyed delete folds absence to `nil`, where
a silent `nil` is indistinguishable from success.

**`ErrInvalidName` lives in `storeapi`, and the store is what enforces it.** A
...
no name-keyed call can address again. `Client` still rejects it up front — that is
...
the column's `CHECK (name <> '')`. A constraint violation arrives as a raw driver
error carrying no sentinel to match on, and its text belongs to the driver; the
contract promises `ErrInvalidName`, so the promise is kept where it can be. The
...
- **Performance is a wash, not a win.** `Get(ctx, name)` is an index probe on the
  existing `UNIQUE ("group", kind, name)` plus a row fetch — two b-tree descents
...
- **The idempotent delete paths got cheaper**, independently: both no-op outcomes now
  ...
  routing all delete traffic through the name path and touching both keyings anyway.
- **An in-memory name→id memo is now safe** but deliberately not taken: the mapping
