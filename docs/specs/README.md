...
others; the dependencies a spec names are the only ordering that binds.

1. [Cache prepared statements](2026-08-20-cache-prepared-statements.md) — ~2–6 µs
   off every statement. Turns on one question: how a statement inside a
   transaction becomes safe to cache.
2. [Reads get their own connections](2026-08-20-reads-get-their-own-connections.md)
   — a writer of one and a reader of N, so a live watch stops slowing writes.
3. [A read-only transaction](2026-08-20-a-read-only-transaction.md) — grouped
   reads move to the reader, where `IMMEDIATE` cannot work. Needs 2.
