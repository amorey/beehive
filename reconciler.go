...
func (t *typedController[Spec, Status]) reconcile(ctx context.Context, id ObjectID) (ReconcileResult, bool) {
	...
	// The baseline is the status as loaded above: what a status write must differ
	// from to be worth a transaction.
	pass := newPassClient[Status](t.bh, t.gk, obj.ID, newStatusBaseline(raw.Status, raw.StatusVersion))
