...
// Nothing in beehive is pushed, so every driver is one of the two shapes here. They
// live together so "what does a non-positive interval mean" is answered once: the
// driver is off, and turning it off is a no-op rather than a panic in
// time.NewTicker.

// runDriver runs step once, then on every tick, until ctx is cancelled or step
...
// stopped immediately.
func runDriver(ctx context.Context, every time.Duration, step func(context.Context) bool) {
	if every <= 0 {
		return
	}
	if !step(ctx) {
...
// tickerChan returns a driver's tick channel and its stop func. It is runDriver's
